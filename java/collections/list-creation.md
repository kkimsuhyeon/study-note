...
> **"싱글톤 패턴"과 같은 단어인 이유** — singleton의 원뜻은 수학 용어 **"원소가 정확히 1개인 집합"**(singleton set). 거기서 갈라져, 디자인 패턴은 "**인스턴스**가 1개"(클래스당 객체 하나) 축에, 컬렉션은 "**원소**가 1개"(리스트 안 값 하나) 축에 같은 단어를 붙였다. 공통 핵심은 "정확히 하나". 재미있게도 `Collections.emptyList()`는 빈 리스트 **공유 인스턴스 하나를 재사용**하니 *패턴* 의미의 싱글톤 — 같은 클래스 안에 두 의미가 다 산다.
