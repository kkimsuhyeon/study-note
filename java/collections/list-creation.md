...
> **"싱글톤 패턴"과 같은 단어인 이유** — singleton의 원뜻은 수학 용어 **"원소가 정확히 1개인 집합"**(singleton set). 거기서 갈라져, 디자인 패턴은 "**인스턴스**가 1개"(클래스당 객체 하나) 축에, 컬렉션은 "**원소**가 1개"(리스트 안 값 하나) 축에 같은 단어를 붙였다. 공통 핵심은 "정확히 하나".
> ⚠️ 단, `singletonList(x)` 자체는 패턴-싱글톤이 **아니다** — 넘긴 값을 담아야 하니 **호출마다 새 객체**가 생긴다(인스턴스 축 ❌, 원소 축 ✅). 반대로 `emptyList()`는 **공유 인스턴스 하나를 재사용**하는 *패턴* 의미의 싱글톤 — 빈 리스트는 상태가 없어 전부 동일 + 불변이라 공유해도 안전하기 때문. (`Collections` 클래스 자체는 그냥 static 유틸 모음 — 싱글턴과 무관)
