...
예시(a → b `REQUIRES_NEW` → c `@Transactional` 합류, c가 RuntimeException):

| b의 처리 | 공유 트랜잭션 | b 커밋 시도? | a로 가는 예외 |
|---|---|---|---|
| c 예외 **잡고 안 던짐** | rollback-only (c 경계에서 찍힘) | O → 못 함 → 롤백 | **`UnexpectedRollbackException`** |
| c 예외 **다시 던짐** | rollback-only | X (예외로 빠져나감) | **원본 RuntimeException** |

> ⚠️ 위 표는 **c가 `REQUIRED`(b에 합류)** 일 때다. **c를 `REQUIRES_NEW`(독립)로 바꾸면** c 실패는 **c 자기 트랜잭션만** 롤백하고 b는 오염되지 않는다 → **b가 catch하면 정상 커밋** ✅. "실패 가능 호출을 독립 트랜잭션으로 떼어 본 작업을 살린다"가 §4 (A)의 실체.
>
> | c의 전파 | c 실패 시 b 트랜잭션 | b가 catch하면 |
> |---|---|---|
> | `REQUIRED`(합류) | rollback-only 오염 | 못 살림 → `UnexpectedRollbackException` |
> | `REQUIRES_NEW`(독립) | 영향 없음 | **정상 커밋** ✅ |
