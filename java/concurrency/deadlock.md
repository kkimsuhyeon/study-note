...
### 사례 2 — 공유락 → 배타락 업그레이드
> **전제: 둘이 "같은 행(row)"을 다룬다.** 같은 행을 둘 다 읽고(S락 공존) 둘 다 그 행을 수정(X락)하려 할 때 생긴다. 서로 다른 행이면 데드락 없음.
```
...
- 서로 **다른 행**(T1=id=1, T2=id=2)이면 충돌 자체가 없어 데드락 안 남.

> 그래서 "읽고 곧 수정할 거면 처음부터 `FOR UPDATE`(배타락)를 써라"가 정설. → 한 명만 읽기 단계에서 진입하므로 업그레이드 충돌 자체가 사라진다.
