...
### `@Version` vs `@Lock(OPTIMISTIC...)` — 토대 vs 보조 옵션

헷갈리는 둘. **`@Version` = 충돌을 감지하는 *데이터*(필수 토대), `@Lock(OPTIMISTIC...)` = 그 감지를 *언제 발동시킬지 조정하는 옵션*.** `@Version`이 없으면 비교할 버전이 없어 `@Lock` 낙관 옵션은 무의미.

| | `@Version` | `@Lock(OPTIMISTIC)` / `OPTIMISTIC_FORCE_INCREMENT` |
|---|---|---|
| 역할 | 버전 컬럼 **선언** = 낙관락의 토대 | 이미 있는 `@Version`을 **특정 조회에 추가 발동** |
| 발동 | 그 엔티티를 **수정(dirty)할 때** 자동 `WHERE version=?` | **읽기만 해도** 커밋 시 버전 검사(/증가) 강제 |
| 필수? | 낙관락의 전제 | 선택 — 드문 특수 상황 |

**왜 `@Lock(OPTIMISTIC)`이 따로 필요? — `@Version`만으론 못 잡는 "읽기 전용 구멍".**
`@Version`은 **그 엔티티를 수정할 때만** 체크한다. 그런데 "**A를 읽기만** 하고 그 값으로 **B를 수정**"하면, A를 update 안 하니 A의 버전 체크가 안 돈다 → 중간에 A가 바뀌어도 모르고 B를 잘못 계산. 이때 `@Lock(OPTIMISTIC)`으로 "읽기만 한 A도 커밋 때 버전 검사"를 강제.
- `OPTIMISTIC` — 읽은 A의 버전이 끝까지 그대로인지 **검사**(증가 X)
- `OPTIMISTIC_FORCE_INCREMENT` — A를 안 고쳐도 버전 **강제 +1** (자식 변경 시 부모 버전도 올려 "애그리거트 전체가 바뀜" 표시)

> 💡 **"돈 조회 = `@Lock(OPTIMISTIC)`"이 아니다.** 트리거는 *돈*이 아니라 **"안 고치는 엔티티인데 내 판단이 그 값에 의존"**하는 접근 패턴. 돈을 *수정*하면 `@Version`(자동) 또는 비관락(고충돌)이고 — 실제로 돈은 "그 행을 직접 차감"이라 **비관락(`FOR UPDATE`)이 더 흔하다.** `@Lock(OPTIMISTIC)`은 "입금(B)하려고 한도(A)만 읽어 검증, A는 안 고침" 같은 read-without-modify에서 A의 무결성을 지킬 때. 일반 수정-충돌엔 **`@Version`만으로 충분**.
...
```
- **왜 비관락이 불가?** 비관락은 트랜잭션 동안 행을 잠그는데, **사람의 생각하는 시간(수 분)을 트랜잭션으로 붙잡을 수 없다.** 그 행을 보려는 모두가 대기 → 마비. **"사람이 끼어 있는 긴 간격"엔 비관락 원천 불가 → 낙관락이 유일한 도구.**
