...
관련 노트: [JPA @Lock 어노테이션](../jpa/lock.md) · [영속성 컨텍스트 · flush · 더티 체킹](../jpa/persistence-context.md) · [ThreadLocal](./thread-local.md)
...
### ⭐ "누가 줄을 세워주나" — 충돌 처리의 주체가 셋 다 다르다

| | 줄 세우는 주체 | 충돌 시 1차 반응 | 앱이 할 일 |
|---|---|---|---|
| 낙관 `@Version` | **없음** (줄 자체가 없다) | 저장 시 예외 (UPDATE 0건 → `OptimisticLockException`) | **수습 전부** — 재시도(@Retryable도 앱 레벨) 또는 사용자 안내 |
| 비관 `FOR UPDATE` | **DB** | **에러가 안 난다** — 늦게 온 쿼리가 그 자리에서 조용히 멈춰 대기, 앞 트랜잭션 커밋 시 풀림. 앱은 "쿼리가 좀 느리네" 정도로만 인지 | 평소엔 없음. 예외는 락 타임아웃·데드락 victim 때만 → 그때만 수습 |
| 분산 Redisson | **클라이언트 라이브러리** | Redis 원시 `SET NX`는 "이미 있음 → 실패 리턴"뿐(줄 세우기 없음). Redisson이 **pub/sub 구독 + 재시도**로 대기를 구현 — unlock 시 메시지 발행 → 대기자 깨어남. waitTime 초과 시 `tryLock`=false | false 받으면 수습 (에러 응답/재시도) |

- "비관락은 재시도 불필요"의 정체 = **DB가 알아서 순서대로 처리**해주기 때문 (앱은 충돌이 있었는지도 모름).
- 분산락(Redisson)은 낙관이 아니라 **비관 전략을 클라이언트 레벨에서 구현**한 것 — "실패를 리턴받는 순간이 있다"는 표면 때문에 낙관과 혼동 주의.
- 낙관 재시도 vs 사용자 안내 기준: **"재시도하면 남의 변경을 덮어쓰나?"** — 재고 차감처럼 새 상태로 재계산하면 재시도 OK, 수정 폼처럼 사람 입력을 통째로 덮으면 "다른 분이 수정했습니다" 안내가 정답.
...
### `@Version` vs `@Lock(OPTIMISTIC...)` — 토대 vs 보조

**`@Version` = 충돌 감지 *데이터*(필수 토대), `@Lock(OPTIMISTIC)` = 그 감지를 "읽기만 한 엔티티"에도 발동시키는 보조 옵션.** `@Version`은 *수정할 때만* 검사하므로, "A를 **읽기만** 하고 그 값으로 B를 수정"하는 **읽기 전용 구멍**은 못 잡는다 → 그때만 `@Lock(OPTIMISTIC)`(검사) / `OPTIMISTIC_FORCE_INCREMENT`(강제 +1).

> 💡 트리거는 *돈*이 아니라 **"안 고치는 엔티티인데 내 판단이 그 값에 의존"**하는 패턴(read-without-modify). 일반 수정-충돌엔 `@Version`만으로 충분. **상세(version 없을 때 예외·세 경우 비교·FORCE_INCREMENT)는 → [@Lock 심화 개념](../jpa/lock-concepts.md) §1·§3.**

### ⭐ 낙관락의 가장 현실적 use case — Optimistic Offline Lock (표시 → think-time → 저장)

"데이터를 **보여주고**, 사용자가 그걸 보고 **수정해서 저장**"하는 흐름(수정 폼)이 낙관락의 교과서 케이스다.
```
화면에 표시 (version=1 같이 읽어 클라이언트로 내려줌: hidden field/ETag)
   ↓ 사용자가 수 분간 폼 수정 (think-time)
저장 요청 (version=1 들고 옴) → UPDATE … WHERE version=1
   ↓ 그 사이 남이 바꿔 version=2 → 0건 → 예외 → "다른 사람이 수정했어요, 새로고침"
```
- **왜 비관락이 불가?** 비관락은 트랜잭션 동안 행을 잠그는데, **사람의 생각하는 시간(수 분)을 트랜잭션으로 붙잡을 수 없다.** 그 행을 보려는 모두가 대기 → 마비. **"사람이 끼어 있는 긴 간격"엔 비관락 원천 불가 → 낙관락이 유일한 도구.** (웹은 애초에 폼열기/저장이 **다른 HTTP 요청 = 다른 트랜잭션**이라 구조적으로도 불가. 억지로 붙잡으면 대기자까지 커넥션을 쥔 채 멈춰 **커넥션 풀 고갈** → 무관한 기능까지 마비 — [transactional.md](../spring/transactional.md) 커넥션 고갈 참고)
- 락은 *표시*가 아니라 **저장(쓰기) 시점**에 작동. 표시 땐 version만 같이 읽어 들고 다니다가, 저장 때 그 version으로 충돌 검사. (Martin Fowler의 *Optimistic Offline Lock*)

#### 💡 일반 낙관락 vs 오프라인 락 — "지키는 시간 창"이 다르다
메커니즘(`UPDATE … WHERE version=?`)은 같고, **version이 여행하는 거리**가 다르다 — 일반은 한 트랜잭션 안(JPA가 서버 메모리에서 관리, 개발자 무관심), 오프라인은 **브라우저까지 갔다 온다**(hidden field/ETag).
- **일반 낙관락**이 잡는 것 = 트랜잭션이 **겹치는 순간**(ms 단위)의 충돌.
- **오프라인 락**이 잡는 것 = "**내가 화면을 본 시점" 이후의 모든 변경** — 동시가 아니어도 잡는다(남이 저장하고 10분 뒤에 내가 저장해도 충돌). 막는 대상이 "동시 접근"이 아니라 **"낡은 화면 기준 덮어쓰기"**.
- 클라이언트가 version을 들고 오는 이유 = **충돌 판정의 기준점**이 "사용자가 본 화면"이 되기 때문. version 없이 저장하면 서버가 최신 version으로 UPDATE → WHERE가 맞아떨어져 **감지조차 못 하고** 남의 변경을 덮어쓴다.
- 사용자에게 강제하는 계약: **"바꿀 거면 새 데이터를 다시 보고 바꿔라."**

> ⚠️ **순수 조회(표시만)는 락이 필요 없다.** v1 읽어 보여준 직후 누가 v2로 바꿔도 "약간 오래된 화면"일 뿐 정상 — 여기에 낙관락을 걸면 "남이 수정했다고 내 *조회*를 실패시키는" 꼴. 조회의 일관 스냅샷이 필요하면 락이 아니라 **트랜잭션 격리수준**(MVCC) 문제. → 낙관락은 "표시→수정→저장"처럼 **결국 쓰기로 이어질 때만** 의미.
