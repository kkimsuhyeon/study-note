...
### Spring 배포 모델로 보면 (실무 감각)

- **Spring Boot 앱 1개 = JVM 1개.** 내장 톰캣이 요청을 스레드 풀로 처리 → 다중 요청도 **같은 JVM 안 여러 스레드** → JVM 락 OK. (빈은 기본 싱글톤이라 모든 요청 스레드가 공유)
- **스케일 아웃 = 사람/인프라가 같은 앱을 N개 인스턴스로 복제**(서버 N대, K8s pod N개…). 스프링이 알아서 JVM을 늘려주지 않는다.
- **로드밸런싱은 스프링 밖**(Nginx / AWS ALB / K8s Service)이 한다. ⚠️ LB는 요청을 **분배만** 할 뿐 **락을 맞춰주지 않는다** — 오히려 같은 키 요청이 서로 다른 인스턴스로 흩어져 JVM 락이 깨지는 원인. → 그래서 분산락.
- **순서/오해 주의**: LB는 서버를 *만들지 않는다*. 인스턴스를 먼저 N개 띄우고 LB가 그 위에서 분배하는 것. "자동으로 인스턴스가 느는" 건 LB가 아니라 **오토스케일러**(K8s HPA / AWS ASG)라는 별개 컴포넌트의 일 — 오토스케일러가 생성/제거하면 LB는 현재 살아있는 것들로 라우팅한다.
- (혼동 주의) "Spring Cloud LoadBalancer"는 *서비스 간 호출*용 클라이언트 측 LB지, 사용자 트래픽 분배나 기본 기능이 아니다.
