...
> ⚠️ **"rich"는 "과함"이 아니라 "좋음"이다.** rich = 엔티티가 자기 로직·불변식을 잘 가진 *좋은* 상태(anemic의 반대). 스펙트럼은 **빈약(anemic, 너무 적음) ── 풍부(rich, 딱 맞음 ✅) ── 과함(over-stuffed, 엔티티가 못 할 일까지)**. "유니크를 엔티티에 넣지 마라"는 *과함*(오른쪽)을 피하라는 것이지 rich를 줄이라는 게 아니다.
