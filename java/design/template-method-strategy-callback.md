...
> 💡 V2가 실무 표준(xxxTemplate)이 된 숨은 이유: **필드가 없다 = 무상태 = 싱글톤 빈으로 안전.** V1에 setter를 달아 전략을 갈아끼우면 싱글톤 필드 상태가 되어 동시 요청이 서로 덮어쓴다(set과 execute 사이에 남이 끼어듦 — [FieldLogTrace 사고](../concurrency/thread-local.md) 재현). 전략이 파라미터로만 흐르는 V2는 이 병 자체가 불가능하다.
