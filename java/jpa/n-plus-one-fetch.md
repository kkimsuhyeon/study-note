...
- **크기는 100~1000 사이에서**: IN절 파라미터를 1000개로 제한하는 DB가 있어 1000이 상한. 크게 잡으면 쿼리 수는 줄지만 순간 부하 증가 — 어차피 전체를 다 읽는다면 **총 메모리 사용량은 같으므로**, DB·앱이 순간 부하를 버티는 선에서 크게. (김영한 활용2)
- **결론 공식**: ToOne은 전부 fetch join(행 안 늘어남 → 페이징 무관) + **컬렉션은 LAZY 유지 + batch size** → 1+N이 1+1로. 페이징이 필요한 목록의 표준 조합.
...
> 📐 **전체 권장 순서 (김영한 활용2 결론)**: ① 엔티티 조회로 시작 — ToOne은 fetch join, 컬렉션은 페이징 필요하면 batch size / 불필요하면 fetch join → ② 안 되면 DTO 직접 조회 → ③ 그래도 안 되면 NativeSQL/JdbcTemplate. 엔티티 방식의 강점은 **코드 수정 없이 옵션(batch size 등)만으로 튜닝 가능**하다는 것. DTO 직접 조회부터는 사실상 SQL을 직접 다루는 세계다.
