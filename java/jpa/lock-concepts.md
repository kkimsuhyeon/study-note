...
> **한 줄 요약**: JPA 락에서 자주 헷갈리는 네 가지 — ① `@Version`만 vs `@Lock(OPTIMISTIC)`, ② 공유 락 vs 배타 락, ③ `OPTIMISTIC` vs `OPTIMISTIC_FORCE_INCREMENT`, ④ `@Version` + 비관적 락 공존.
...
두 트랜잭션이 같은 행에 공유락을 동시에 잡은 뒤 둘 다 UPDATE(배타락)로 올리려 하면, 서로의 공유락 해제를 기다려 **데드락**에 빠진다. (시간순 다이어그램·예방은 → [데드락 §3 사례2](../concurrency/deadlock.md))
그래서 **"읽고 나서 곧 수정할 거면 처음부터 `FOR UPDATE`를 써라"**가 정설. 읽기 정합성만 필요하면 보통 일반 SELECT(MVCC)나 낙관적 락(`@Version`)으로 해결하고, 공유락은 FK 보호 등 제한적 상황에만 신중히 쓴다.
...
> **⚠️ 실무 빈도**: `*_FORCE_INCREMENT`(낙관/비관 둘 다)는 **거의 안 쓴다.** "엔티티 자체는 안 바뀌는데 자식 변경 때문에 부모 version을 올린다"는 DDD 집합체 특수 요구에만 필요해서, 실무에서 평생 한 번도 안 써보는 경우도 많다. 현실 선택은 대부분 **`@Version`(낙관, 일반 수정) / `PESSIMISTIC_WRITE`(배타락, 재고·좌석·잔액) / 조건부 UPDATE(단순 차감)** 셋으로 끝난다. (`PESSIMISTIC_WRITE`가 비관적 락의 사실상 표준)

---

## 4. `@Version`이 있는데 비관적 락을 쓰면? (공존 가능, 낙관적 검사는 무의미해짐)

**결론: 같이 써도 된다. 둘은 배타적이지 않다.** 다만 낙관적 락의 "충돌 감지" 역할이 **비관적 락에 묻혀 사실상 무의미**해진다(사라지는 게 아니라 실패할 수가 없어짐).

- 비관적 락(`FOR UPDATE`)은 읽는 순간 row를 잠가 **동시 수정 자체를 차단**한다 → 이게 실질 보호.
- 그 상태에서도 `@Version`은 커밋 시 version을 +1 하고 `WHERE version = ?`도 붙인다. 그러나 **이미 비관적 락이 동시 수정을 막았으니 version 조건이 틀릴 일이 없다.** 검사는 돌지만 항상 통과.

```sql
SELECT ... FROM product WHERE id = 1 FOR UPDATE;   -- ① 비관적 락(실질 보호)
-- 다른 트랜잭션은 여기서 막혀 version을 못 바꿈
UPDATE product SET stock = 9, version = 2 WHERE id = 1 AND version = 1;  -- ② version 검사는 돌지만 안 틀림
```

> 즉 "낙관적 효과가 사라지나?" → **충돌 감지 역할이 무의미**해질 뿐, version 컬럼은 계속 증가하므로 **다른 낙관적 경로에는 여전히 신호**로 작동한다.

### 그럼 왜 같이 쓰나 (의미 있는 경우)

| 경우 | 설명 |
|------|------|
| **혼합 접근** | 엔티티에 `@Version`이 이미 있고 대부분 낙관적으로 접근하지만, **특정 경로만 비관적**으로 강하게 잠금. `@Version`을 떼지 않아도 됨(비관적 경로에서도 version 정상 증가). |
| **`PESSIMISTIC_FORCE_INCREMENT`** | DB 락으로 독점 + **version 강제 +1**. 안 고쳐도 version을 올려 다른(낙관적) 관찰자에게 "나 바뀌었다" 신호. (자식 변경 → 부모 버전 올리기를 비관적으로) |
| **락 업그레이드 시 version 검사** ⚠️ | `@Version` 엔티티를 **락 없이 먼저 읽고**, 나중에 같은 엔티티에 비관적 락을 걸면 그 시점에 Hibernate가 **version을 한 번 검사**한다. 그 사이 남이 바꿨으면 `OptimisticLockException`. → "비관적 락을 늦게 걸면" 낙관적 검사가 실제로 발동할 수 있음. |

> 한 줄: 공존 OK / 같은 경로에선 낙관적 충돌검사가 비관적 락에 의해 실패 불가(무의미) / version 증가·신호 역할은 유지 / `FORCE_INCREMENT`·락 업그레이드에선 의미가 생긴다.
