...
1. 획득 실패한 대기자는 `redisson_lock__channel:{락키}` 채널을 **구독**하고 잠든다 ([Redis Pub/Sub](./redis-pubsub.md))
2. 주인이 `unlock()` — 역시 Lua: 재진입 카운트 -1, 0이 되면 `DEL` + 채널에 해제 메시지 **publish**
3. 방송을 들은 대기자들이 깨어나 다시 tryLock Lua를 시도 — 여기서도 Redis가 먼저 처리한 하나만 성공
