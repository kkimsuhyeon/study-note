...
- **현재 위치 (2026-08-18)**: 트랙 10(스프링 프록시→AOP) 복습 진행 중 — "스프링 기본기 같이 배우기" 모드(챕터마다 스프링 연결 고리 문항 포함). **ThreadLocal 챕터 완료(Q27·Q28)**, 8/18 주간 회독 1회차(동시성 W1~W3) 완료, **템플릿 메서드/전략/콜백 챕터 완료(8/24, Q29~32 — 🌀1회독 라이트 모드 첫 적용, 4/4 통과)**. 다음 챕터: 프록시/데코레이터([proxy-decorator-pattern.md](java/design/proxy-decorator-pattern.md), 1회독 모드). 사용자 목적 확인(8/18): ①노트 전체 지도 인지 ②면접 준비(말로 설명 연습 포함) ③실전 감각 — 면접 각도 변형·챕터말 지도 연결·노트에 없는 내용 적극 보강+보고.
...
- **페이싱 조정 (8/24, 사용자 피드백)**: 심화 갈래(예: pinning 데드락 승격)가 나오면 **"🔬 심화 — 노트 캡처만, 암기 대상 아님" 라벨**을 붙여 필수/심화를 구분할 것. 사용자가 "기본도 모르는데 너무 깊이 가나" 부담 표현 → 필수 라인(면접 대비 3줄 수준)과 심화를 명시적으로 분리, 단계적 진행.
- **🌀 3회독 체제 (8/24 신설, 사용자 제안)**: 목표 = 볼트 전체(~50노트) 커버. **1회독**: README 트랙 순서대로 노트당 확인 문제 1~2개(한 줄 요약·"뭘 다루나" 수준, 심화 금지) — 세션당 5~8노트, 전체 지도 깔기 우선. **2회독**: 같은 순서로 ⚠️함정·메커니즘 깊이. **3회독**: interview-questions.md 기반 꼬리물기 체인. 오답 스윕(세션 시작)은 전 회독 공통 유지, 주간 회독은 1회독 진행에 흡수. 트랙 10 잔여 챕터부터 1회독 모드로 전환.
...
## 2026-08-24 — 자율 회독 후 질문 (pinning 심화)
- 사용자가 5일 만에 자율 전체 회독 후 질문 — **pinning 이름+메커니즘 자기 말 재진술 성공 ✅** (W3 재출제 항목 일부 통과). 질문 "긴 DB가 문제? 데드락?" → 제3범주 해설: **예외도 에러도 아닌 조용한 확장성 저하**, 피해=핀 시간×빈도×동시 수, 블로킹 끝나면 자연 해소(데드락과의 구분: 원형·영원 vs 일방·시한부, Q17 구분 재사용). + 최악엔 **진짜 데드락 승격**(JEP 491 동기: Java 21은 monitor 진입 대기도 pinning → 락 보유자가 돌아올 캐리어를 대기자들이 점거) → [virtual-threads.md §5](java/concurrency/virtual-threads.md) 보강. W3 잔여 확인 항목: Java 21 처방(ReentrantLock)·JEP 491로 뭐가 바뀌었나.
- 후속: "캐리어 전멸" 표현 혼란 → **만석**(죽음 아님, 빈 버스 0대·시한부)으로 교정. 데드락 승격 구조는 사용자가 자기 말로 재구성 성공 ✅("나올 때도 캐리어가 필요하네"). 사용자 페이싱 피드백 → 심화 라벨링 도입(인수인계 참조), 필수 라인 3줄(throughput+Semaphore / pinning 정의 / 처방 ReentrantLock·JEP491) 확정. 처방은 해설로 제공됨 — 다음 스윕에서 회상만 확인.
- ⚡ 8/24 스윕(3.5/4): S1 no Session=영속성 컨텍스트 ✅(**Q26② 통과 — proxy 챕터 종료**) / S2 lost update 이름 ✅(2회 실패 후 성공, "누락=lost" 앵커 정착) / S3 finally ✅·위치 이름(필터/인터셉터 afterCompletion) 미회상 → 재예약("관문=필터" 앵커) / S4 Semaphore+허가증 동작 ✅(**W2 통과** — ① 사슬은 8/19 파이프라인 재구성으로 검증됨).
- 사용자 페이싱 재확인: "천천히 쭉 돌아가면서" — **이후 문항은 한 번에 1개, 가볍게** (4개 몰아내기 금지).
- 잔여 스윕: ThreadLocal 정리 위치 이름(필터/인터셉터 afterCompletion), W3 처방(ReentrantLock·JEP491) 회상, 가시성↔원자성 구분 확인.

## 2026-08-24 — 트랙 10 재개: 템플릿 메서드/전략/콜백 (🌀1회독 라이트 모드 첫 적용)
- 페이싱 최종 합의(8/24~25 왕복 조정): 깊이 기준 확정 — **메커니즘·판단형·시나리오 문제가 기본**(ThreadLocal Q28·쿠폰 W1 수준). 요약 확인만 하는 라이트 모드는 너무 얕음(사용자 피드백), JEP 문서급 심화 갈래는 너무 깊음(🔬라벨로 노트 캡처만). 한 턴에 한 문제. 1회독의 의미 = "노트당 문제 수를 줄여 전체를 빨리 돎"이지 "얕게"가 아님.
- [x] **Q29. 분리 대상** — "서비스 로직 vs 공통 로직" ✅. 용어 사슬 연결: 변하는/변하지 않는 것 → 핵심/부가 기능 → (AOP에서) 횡단 관심사.
- [x] **Q30. 템플릿 메서드를 버린 이유** — 단일 상속·강결합 ✅ ("부모 기능 안 쓰는데 인질" 구체화).
- [x] **Q31. 전략 V1 vs V2** — 결정 시점(조립 시/실행 시)·DI=V1·JdbcTemplate=V2 전부 ✅. "선 조립 후 실행" 명명 보충.
- [x] **Q32. 템플릿 콜백 + 챕터 한계** — ①함수형 인터페이스 **이름 자발 회상** ✅, 정의 교정: "구현체 1개"→**추상 메서드 1개**(람다=유일한 구현으로 추론 가능) ②"원본에 execute가 남는다" ✅ → 다음 챕터=프록시. **챕터 완료 4/4** — 8/12 학습분 12일 후에도 유지 확인. 면접 은행에 3문항 추가.

### 8/25 — 깊이 재조정 후 보강 라운드 (함정·판단형 3문제)
- [x] **Q33. 예외 삼키기 리뷰(판단형)** — "실패가 성공으로 둔갑, 사용자는 완료 화면" 핵심 시나리오 자력 도달 ✅. 보충 2피해: **롤백 무산**(예외가 프록시 경계까지 올라와야 롤백 — Q22 역방향)·null 전파 NPE(현장 이탈). 원칙 "부가 기능은 관찰자, 심판 아님" → 노트 §4 피해 사슬 보강.
- [x] **Q34. ContextV1 setter 동시성(연결형)** — 싱글톤 1개·전략 덮어씀 구조 파악 ✅. 타이밍 시나리오(A set→B set→A execute=B 전략 실행)와 **FieldLogTrace 동일 병** 연결은 해설로 완성. V2=무상태=싱글톤 안전(xxxTemplate이 싱글톤인 이유) → 노트 §2-3 💡 캡처.
- [x] **Q35. 람다 캡처 effectively final** — 힌트(스택·다른 스레드) 후 **"복사해서 갖고 있나?" 스스로 도달** ✅. 금지 이유(원본-복사본 불일치 방지)·해법(새 변수) 해설 → 노트 §4 보강. 면접 은행: effectively final 꼬리 + 예외 삼키기 문항 추가.
- ⭐ 파생 문답(Q35 후속, 스스로 파고듦): "람다도 자체 스택 갖나?" → 교정: 스택은 **스레드 소유물**, 람다는 실행 스레드 스택에 프레임만 빌림. **람다=객체 → 캡처본은 힙의 람다 객체 필드** — 힙으로 복사하는 이유는 힙이 공유지라 딴 스레드도 실행 가능("스레드가 힙 공유한댔나"를 스스로 연결 ✅, Q1 힙/스택 구분이 캡처 설계 이유까지 설명) → 노트 §4 저장 위치 보강.
- 페이싱 최종 확정: 이 보강 라운드 깊이(메커니즘·판단·연결형)가 기준. **템플릿 챕터 최종 완료(본 4 + 보강 3 + 파생 1).**
...
- [x] **Q26. 프록시 메커니즘** — ①프록시 ✅ ③에러 방향 ✅(예외 이름은 몰랐음). ②초기화 메커니즘 처음 배움: 상속한 가짜(id+target), 메서드 가로채기 → **영속성 컨텍스트에 초기화 요청** → target 연결·위임. ③=LazyInitializationException("no Session") — 영속성 컨텍스트가 닫혀 요청할 곳이 없어서. OSIV ON이 이걸 가리는 이유까지 연결. [③ 이름은 8/18 L5 통과, ②는 8/24 스윕 "no Session=영속성 컨텍스트" 회상 성공 — **통과, 챕터 종료**.]
